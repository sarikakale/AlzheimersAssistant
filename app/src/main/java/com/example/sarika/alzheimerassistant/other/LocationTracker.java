...
public class LocationTracker implements
    ...
    public LocationTracker(Context context, LocationCallback callback, int delayInSeconds) {
        mGoogleApiClient = new GoogleApiClient.Builder(context)
                .addConnectionCallbacks(this)
                .addOnConnectionFailedListener(this)
                .addApi(LocationServices.API)
        ...
        mLocationRequest = LocationRequest.create()
                .setPriority(LocationRequest.PRIORITY_HIGH_ACCURACY)
                .setInterval(delayInSeconds * 2 * 1000)        // 10 seconds, in milliseconds
                .setFastestInterval(delayInSeconds * 1000); // 1 second, in milliseconds
